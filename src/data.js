...
export const contractImpactCards = [
  {
    title: "Что уже можно использовать без редеплоя DSF",
    subtitle: "Опирается на текущие функции контракта как есть.",
    items: [
      "Текущий режим отложенного депозита и вывода уже существует через feesOptimizationDeposit, feesOptimizationWithdrawal и операторское завершение.",
      "Протокол уже может оплачивать транзакции завершения батча со стороны оператора, потому что эти вызовы делает роль OPERATOR_ROLE.",
      "Можно строить интерфейс, который объясняет пользователю модель батча и ожидания без изменения основного контракта."
    ],
    conclusion: "Но пользователь все равно должен сам создать исходную заявку в сети и заплатить за нее ETH."
  },
  {
    title: "Что можно сделать без редеплоя DSF, но через внешний слой",
    subtitle: "Потребуются отдельные обвязочные контракты или сервисы, но сам DSF можно оставить как есть.",
    items: [
      "Можно сделать внешний контракт-обертку для депозита: он получает стейблкоины пользователя по подписи, затем вызывает DSF.deposit от своего имени и передает LP-токены пользователю.",
      "Можно сделать внешний контракт-обертку для вывода: он принимает LP-токены от пользователя, вызывает DSF.withdraw от своего имени и затем пересылает полученные стейблкоины пользователю.",
      "Можно внедрить собственное кошелек-приложение на ERC-4337: тогда умный кошелек пользователя будет напрямую вызывать текущий DSF, а оплату комиссии можно вынести в спонсирующий слой."
    ],
    conclusion: "Это сильный путь, если нужно быстро обернуть текущий DSF более удобным слоем, не трогая основной контракт."
  },
  {
    title: "Что требует изменения и редеплоя DSF",
    subtitle: "Это уже не внешняя обвязка, а изменение базовой логики самого протокола.",
    items: [
      "Нативный депозит или вывод по подписи внутри самого DSF.",
      "Поддержка доверенного посредника по ERC-2771 через корректный _msgSender().",
      "Поддержка собственного разрешения на списание или специальных методов depositBySig / withdrawBySig.",
      "Изменение токена DSF LP, если нужен вывод без отдельного предварительного разрешения на передачу LP-токенов."
    ],
    conclusion: "Если цель — чтобы сам DSF понимал подписи и сам умел работать как gasless-протокол без внешней обвязки, редеплой практически неизбежен."
  }
];

export const contractImpactNotes = [
  {
    title: "Ключевое ограничение текущего DSF.sol",
    text: "Функции deposit и withdraw завязаны на _msgSender(). Это значит, что контракт ожидает, что именно вызывающий адрес владеет активами и получает результат. Для обычного relayer-подхода этого недостаточно."
  },
  {
    title: "Почему ERC-4337 здесь особенный случай",
    text: "Если DSF делает собственное кошелек-приложение, то умный кошелек сам становится реальным владельцем активов и вызывающим адресом. Поэтому ERC-4337 может дать стратегический путь без обязательного редеплоя самого DSF."
  },
  {
    title: "Что реально не получится без изменений контракта",
    text: "Нельзя сделать полностью нативный withdraw-by-signature внутри DSF, потому что в текущем виде контракт не умеет проверять подпись пользователя и исполнять вывод за него напрямую."
  }
];
